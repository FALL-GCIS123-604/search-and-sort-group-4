...
def insertion_sort(small_data):  
    for i in range(1, len(small_data)): #for loop to start from index 1 and compare and go up till length of data based on indexes, start from index 1 because we assume index0/element 1 is sorted
        ...
        j = i - 1 #identifies j must be one index less than i, in this case index 0 (sorted elemnt)
        while j >= 0 and temp < small_data[j]: 
            small_data[j + 1] = small_data[j] # if temp value is less than j, j moves foward by one index
...
def merge_sort(array):
    if len(array) > 1:
        middle_index = len(array) // 2  # Find the middle point
        left_half = array[:middle_index]  # Split the array into left and right halves
...
# phase 4: Compare Search Times
compare_search_times(large_data, 72)